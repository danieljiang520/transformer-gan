# The following codes are gently revised based on HuggingFace Transformer language-modeling
...
def mask_tokens(inputs: torch.Tensor, tokenizer: PreTrainedTokenizer, args) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    special_tokens_mask = [
        [0] * len(val) for val in labels.tolist()
    ]
...
def train(args, train_dataset, model: PreTrainedModel, tokenizer: PreTrainedTokenizer) -> Tuple[int, float]:
    ...
    # Check if  training from a checkpoint
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    ...
                    tr_loss_log = (tr_loss - logging_loss) / args.logging_steps
                    tb_writer.add_scalar("loss", tr_loss_log, global_step)
                    tr_perplexity = torch.exp(torch.tensor(tr_loss_log))
                    tb_writer.add_scalar("perplexity", tr_perplexity, global_step)
...
def evaluate(args, model: PreTrainedModel, tokenizer: PreTrainedTokenizer, prefix="") -> Dict:
    ...
    with open(output_eval_file, "w") as writer:
        ...
        for key in sorted(result.keys()):
            ...
            writer.write("%s = %s\n" % (key, str(result[key])))
            print("Evaluation:   %s = %s".format(key, str(result[key])))
